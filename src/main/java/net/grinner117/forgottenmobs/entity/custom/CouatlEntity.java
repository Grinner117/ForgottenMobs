...
public class CouatlEntity extends FlyingMob implements Enemy, GeoEntity {
    ...
    private PlayState predicate(AnimationState animationState) {
        if (animationState.isMoving()) {
            animationState.getController().setAnimation(RawAnimation.begin().then("animation.couatl.walk", Animation.LoopType.LOOP));
            return PlayState.CONTINUE;
        }

        animationState.getController().setAnimation(RawAnimation.begin().then("animation.couatl.idle", Animation.LoopType.LOOP));
        return PlayState.CONTINUE;
    }
    ...
    private PlayState attackPredicate(AnimationState state) {
        if (this.swinging && state.getController().getAnimationState().equals(AnimationController.State.STOPPED)) {
            ...
            state.getController().setAnimation(RawAnimation.begin()
                    .then("animation.couatl.attack", Animation.LoopType.PLAY_ONCE));
            this.swinging = false;
        }
